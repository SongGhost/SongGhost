...
export default function OnboardingModal({
  ...
  // Never auto-dismiss Step 2 (Connect Spotify) — that step is action-opened only.
  useEffect(() => {
    if (!open || !isSignedIn) return;
    if (targetStep === 2) return;
    onContinueAsGuest?.();
  }, [open, isSignedIn, targetStep, onContinueAsGuest]);
  ...
  return createPortal(
    ...
    >
      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-accent/30 bg-[#121214]/95 px-6 py-9 shadow-[0_0_48px_rgba(41,146,207,0.14)] sm:px-8">
          ...
          <div className="mt-8 flex flex-col gap-3">
            {!isSignedIn && (
              <SignInButton mode="modal">
                <button
                  type="button"
                  className="inline-flex min-h-12 w-full items-center justify-center rounded-lg bg-accent px-4 py-3 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-950 transition hover:bg-accent-hover"
                >
                  Sign in / Create account
                </button>
              </SignInButton>
            ...
            {onContinueAsGuest && (
              <button
                ...
                className="inline-flex min-h-12 w-full items-center justify-center rounded-lg border border-white/[0.12] bg-zinc-900/60 px-4 py-3 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-100 transition hover:border-accent/40 hover:text-accent"
              >
                Continue as Guest
              </button>
            )}
        ...
        {showSpotifyStep && onContinueAsGuest && (
