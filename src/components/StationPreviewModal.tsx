...
type StationPreviewModalProps = {
  open: boolean;
  onClose: () => void;
  tracks: StationTrack[];
  onPlay: (tracks: StationTrack[]) => void;
  stationName?: string;
...
export default function StationPreviewModal({
  ...
  const [queue, setQueue] = useState<StationTrack[]>(tracks);

  const [saveOpen, setSaveOpen] = useState(false);
  const [stationName, setStationName] = useState("");
  const [stationPersonaId, setStationPersonaId] = useState(() =>
    getPersonaPickerValue(undefined, isPro),
  );
  ...
  useEffect(() => {
    ...
    if (!allowed.has(stationPersonaId)) {
      setStationPersonaId(
        getPersonaPickerValue(undefined, isPro) || personaOptions[0]?.id || "sam",
      );
    }
  }, [saveOpen, isPro, personaOptions, stationPersonaId]);
  ...
  const openSaveForm = () => {
    ...
    setCoverError(null);
    setStationName(initialStationName?.trim() ?? "");
    setStationPersonaId(getPersonaPickerValue(undefined, isPro));
    setSaveOpen(true);
  };
  ...
  const addPending = (mode: "next" | "append") => {
    ...
    setQueue((prev) =>
      mode === "next" ? [pendingTrack, ...prev] : [...prev, pendingTrack],
    );
    setPendingTrack(null);
    setSearchQuery("");
    setSearchOpen(false);
  ...
  const handleDrop = (e: React.DragEvent, index: number) => {
    ...
    e.preventDefault();
    if (dragIndex !== index) {
      setQueue((prev) => reorderLocalQueue(prev, dragIndex, index));
    }
    endDrag();
  };
  ...
  const handleGripKeyDown = (e: React.KeyboardEvent, index: number) => {
    ...
    setQueue((prev) => reorderLocalQueue(prev, index, target));
  };
          ...
          {queue.length === 0 ? (
            ...
            <ol className="space-y-1">
              {queue.map((track, index) => {
                ...
                const isDropTarget =
                  dragIndex !== null && dropIndex === index && !isDragging;
                ...
                return (
                  <li
                    ...
                    onDragEnd={endDrag}
                    className={`group flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs sm:text-sm transition-all duration-150 border border-transparent hover:bg-[#ECE8DF]/80 ${
          ...
          {saveOpen ? (
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                <label htmlFor="preview-saved-station-name" className={fieldLabelClass}>
                  Station Name
                </label>
                <input
                  id="preview-saved-station-name"
              ...
              <div className="space-y-1">
                <label htmlFor="preview-saved-station-persona" className={fieldLabelClass}>
                  DJ Persona
                </label>
                <select
                  id="preview-saved-station-persona"
