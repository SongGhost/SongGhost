 ...
 * Studio mixes gate on Spotify / Apple Music; catalog & saved stations offer
 * Listen on SongHost + Save to My Radio.
 */
export default function PublicStationPlayer({
  ...
  const isStudioShare = Boolean(ready);
  ...
  const station = useMemo(() => {
    if (ready) return studioManifestToStation({ ...ready, djConfig });
    return playableStation;
  }, [djConfig, playableStation, ready]);
   ...
   */
  useEffect(() => {
    if (!isStudioShare || !hasStreamingSession) {
  ...
  const isPlaybackActive = Boolean(
    hasTunedIn &&
      (companionPlayback?.isPlaying || companionActive || isTuningIn),
  );
  ...
  const tuneIn = useCallback(async () => {
    ...
    try {
      ...
      setTuneError(err instanceof Error ? err.message : "Failed to tune in");
    } finally {
      setIsTuningIn(false);
    }
  }, [
  ...
  // Catalog / saved station landing — SongHost YouTube player via home deep link.
  if (publicStation && !isStudioShare) {
    const featuring =
      publicStation.seedArtists.length > 0
        ? publicStation.seedArtists.join(", ")
        : publicStation.genres.join(", ");
    const listenHref = `/?station=${encodeURIComponent(publicStation.id)}`;
      ...
      <div className="relative flex min-h-screen items-center justify-center bg-[#09090b] px-4 text-zinc-100">
        ...
        <main className="relative mx-auto w-full max-w-lg text-center">
          ...
          <p className="mx-auto mt-4 max-w-md font-sans text-base leading-relaxed text-zinc-400">
            Hosted by {creatorName}. A custom AI radio station featuring{" "}
            {featuring}.
          </p>
  ...
  // Studio connection gate — Spotify Premium / Apple Music required.
  ...
  // Authenticated studio player view
  return (
          ...
          <div className="space-y-8">
            <div className="flex flex-col items-center gap-5">
              ...
              {(tuneError || companionNotice) && (
                <p
                  className="max-w-sm text-center font-sans text-sm text-red-300"
                  role="alert"
                >
                  {tuneError ?? companionNotice}
