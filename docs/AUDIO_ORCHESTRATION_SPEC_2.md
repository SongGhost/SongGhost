...
**Copy cadence (pace → opener):** Host Studio pace is forwarded into every generate-script path (`pace` / `chatterPacing` / `talkLevel`). Every Song mid-session must **lead with what's next** (`up next` / `now playing` / `here's`) and must **not** default to “You just heard…” / “That was…”. Natural Pace and Long Breaks **may** open with “You just heard / That was” when bridging after silent songs. Pass 4 first-playlist Song 1 → Song 2 pack stays the one-shot “That was … Up next …” pair. Catch-up `recap` stays lookback. `promptBuilder.buildCopyCadenceDirective` / `paceGuidance` encode the rule.
...
**Persona quality gate (MUST):** After script generation, `runPersonaScriptQualityGate` may trigger **one** retry if a teaching clip misses the required move, uses banned filler as empty hype, or trips the clone-opener heuristic. If the retry still fails, air the best attempt — never hang the gap, never talk over music. Local TTS remains fail-closed. Do not validate session-opening templates, `song_id`, stingers, recaps, or names-only announcement clips.
