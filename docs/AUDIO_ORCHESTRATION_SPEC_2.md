...
- **`intro_ramp` (canonical default):** element volume is pre-set at `DUCK_RATIO = 0.18` from time `0:00` before any audible frame. Unlock / clean-start MAY play, but only after `setDuckGain(DUCK_RATIO)`. Host speaks over the ducked bed; on speech end the bus swells to `1.0` without pausing or seeking.
...
| Method | Contract |
|--------|----------|
| `setLaunchHold(active, mode = "hard_pause")` | Arms or releases `launchHoldActive` / `launchHoldMode`. Does **not** flip `intendedPlaying`. AudioPlayer session arm passes `"intro_ramp"` (canonical opener default). `intro_ramp` immediately `setDuckGain(DUCK_RATIO)` **once** then `applyLaunchHold()`. Subsequent `playing` / `ensurePlayback` / `applyUnlock` / position ticks MUST NOT re-pin duck. |
...
| `GET /api/song-radio` | `catalogPreviewUrl` / `attachSeedCatalog` require `itunesTitlesMatch` **and** `itunesArtistsMatch` against the requested seed before binding a preview. Seed catalog is `lookupITunesSongById(id, seedIdentity)` then `lookupITunesTrack(artist, title)` — never an unverified first-result attach. `resolveCandidate` MUST NOT stamp YouTube IDs (`youtubeId: ""`); HTTP `previewUrl` / `streamUrl` only; drop rows with neither so `resolveDirectStreamUrl` attaches `DirectStreamProvider` (native HTML5 `<audio>`) for Pocket Mode. Queue tail is a Last.fm / MusicBrainz similar-artist mix (same `fetchSimilarArtists` path as Artist Radio), not a same-artist iTunes fallback. Empty similar pool or fewer than 2 unique primary artists → **404**. |
