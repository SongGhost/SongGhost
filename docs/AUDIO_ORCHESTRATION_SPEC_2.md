...
`GET /api/studio/voice-preview?personaId=…` is the only Studio audition path.

| `personaId` | Engine | Fail-closed rule |
|-------------|--------|------------------|
| OpenAI voice (`onyx`, `cedar`, …) or persona id | Existing OpenAI (or mothballed ElevenLabs) preview; disk cache allowed | Unchanged |
| `local:1` … `local:4` | `synthesizeLocalSpeech` → sidecar `POST /v1/speech` with that `voiceSlot` | **MUST NOT** synthesize or cache OpenAI audio. Sidecar down / `LOCAL_TTS_URL` unset / empty bytes → JSON `{ error, code: "LOCAL_TTS_UNAVAILABLE" }` and a visible Host Studio error (“start the local voice helper”). `Cache-Control: no-store`. |

Host Studio (`HostVoicePersonaSelector`) fetches the preview as a blob and plays it. Selecting an OpenAI card stores that OpenAI id on `preferredVoice`. Selecting Custom 1–4 stores `local:N`. The live dial calls `resolvePreferredVoiceTarget()` / `resolveLiveHost()` → `{ provider: "local", voiceSlot }` and uses that pick for the session opener and between-song breaks.

### 4.1 TTS Input Sanitization

**SSML Stripping:** Strip or convert all XML / SSML tags (e.g. `<break time="..."/>`, `<say-as>`, and other markup) into natural punctuation (commas, periods, or ellipses) **before** dispatching text to TTS engines that do not support raw XML payloads (including ElevenLabs REST, OpenAI `gpt-4o-mini-tts`, and the local sidecar). Raw SSML MUST never appear in the synthesis request body. `prepareTtsSynthesisText()` applies the same strip/ellipsis path to every provider.
...
**Fail-closed voice integrity (MUST):** `/api/generate-script` and `/api/generate-voice` MUST synthesize the Host Studio pick: `preferredVoice` when it is an OpenAI id (else persona default) with that persona's `ttsInstructions`, or `provider: "local"` + `voiceSlot` when the pick is `local:N`. On the mothballed ElevenLabs path, `400` / `402` / `429` (or a complete engine fault) MUST NOT fall through to a female premade (Rachel), a different persona, or a generic OpenAI voice while claiming the locked host. Fail the DJ break instead so music continues without a voice jump. On `provider: "local"`, a missing `LOCAL_TTS_URL`, sidecar fault, or GPU/model miss MUST likewise fail closed — no silent OpenAI fallback and no Phase A beep. Host Studio custom-slot audition (`/api/studio/voice-preview` with `local:N`) uses the same fail-closed rule. Prefetched clips MUST match `activePersonaId` / `activeVoiceId` before playback; stale or mismatched buffers are discarded. Live dial sends `provider: "openai"` or `"local"` from the Host Studio pick.
