...
### YouTube first-song invariant & offscreen host (current dial)
...
| **ControlDeck dock** | `fixed bottom-0 inset-x-0 z-50 bg-[#09090b]/92 backdrop-blur-xl pb-[env(safe-area-inset-bottom)]`. Holds transport, Host Studio pill, Host Controls drawers, pinned mobile `trackActions`, and `{children}`. There is **no** `MusicSourceHeader` component. `ControlDeck` mounts only `DevTierBadge` from `src/components/header/Header.tsx`; the default export of that file (Connect Music / `MusicSourceModal`) is **not imported**. `DevTierBadge` always renders **YT View** (test-only dock viewer) to the left of Free/Pro. In development (or `NEXT_PUBLIC_ENABLE_DEV_TOGGLE=true`), it also renders **Full Songs (Dev)** beside Free/Pro. |
| **Dashboard column** | `page.tsx` content uses `pb-32` / `md:pb-36` so carousels clear the dock (extra ~220px when **YT View** is on). `HeavyRotationShelf` (`src/components/dashboard/HeavyRotationShelf.tsx`) exists but is **not imported** on home. `shouldAutoStageHeavyRotation()` returns **`false`**. Post-Clerk boot MUST NOT auto-open Step 2 ("Connect Spotify"). |
| **`{children}` slot** | Unconditionally mounted inside the dock wrapper. Contains `<AudioPlayer>` (seek bar + YouTube host + DirectStream media element). MUST NOT be gated on idle, sheet-open, `md` breakpoint, or Host Studio open. A remount would reset `useStationQueue` (`stationId` / `queueGeneration` effects). |
...
| **YouTube host** | Default: `fixed -left-[9999px]` 320×180 `opacity-0` in `AudioPlayer.tsx`. Test-only **YT View** restyles the same node in-flow at 320×200 (`setSize`) without remounting. |
