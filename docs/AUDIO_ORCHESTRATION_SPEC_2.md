...
Two-ahead break prefetch from **song start**. While a song plays, keep two ready break packages (lore clip + handoff/announcement as today's structure). Start fetching when the song starts, not near the end. Local GPU: one synth job at a time; finished packages stay ready as the queue drains. OpenAI may run up to two prefetch jobs in parallel and does not occupy the local slot. Skip / new station-playlist / Host Studio settings that change script or voice: abort in-flight (Pass 1), drop stale packages (fingerprint miss), immediately retarget the new upcoming two. No Director's Cut length soft-cap. Natural Pace / non–Every Song cadence unchanged after the one-shot first-playlist pack. Miss at transition: Pass 2 fallback if still in the gap, else clean skip → song at 100%. Pass 4 first-playlist "that was / up next" is the Song 1 → Song 2 pack only.

### Change log — v3.15.2 (Sep 20 2026)

Prerecorded Custom welcome + fallback banks. Harris/Piper/Quinn/Bea (`local:1`–`4`) play a random unused-recent **welcome** on new playlist/station/listen start (gap, then Song 1 at 100%). When a live break is not ready, times out (20s), or aborts while the gap is still open, play a short **fallback** instead of dead air — Pass 1 refuse-late-play still wins if music already started. Scripts: `src/lib/audio/prerecorded/scripts.json`. WAVs: `public/audio/prerecorded/slot-{N}/` generated by `npx tsx tools/local-tts-sidecar/generate-prerecorded.ts`. OpenAI-selected voices skip this bank this pass.

### Change log — v3.15.1 (Sep 20 2026)

Never late talk-over. Each live-dial break attempt has a generation token minted **before** any TTS await. Skip, station/playlist change, and Host Studio settings that affect script/voice abort `/api/generate-voice` and the local sidecar job (`POST /v1/cancel` + disconnect). If the voice package is not on-air within **20s** (`VOICE_PACKAGE_DEADLINE_MS`), skip the break and start the song at 100%. `VoiceNode.play()` refuses a stale generation or a clip after launch-hold release. CUDA cannot be preempted; cancel discards the WAV so a late clip cannot air.
...
**PREFETCHING_BREAK:** Fetching script from `/api/generate-script` and downloading/synthesizing TTS audio blob. DirectStream does **not** route on companion Mode A vs Mode B. Mid-session music keeps playing at full gain until the voiced break pauses it. Two-ahead warmup starts at song start (N+1 and N+2). Warmed clips are **off-graph** (`VoiceNode.preload`) — they MUST NOT attach to the live session `AudioContext` or `MediaElementAudioSourceNode`. Prefetch completion is **not** on-air (see TRACE 4 split). Track 1 session openers skip this prefetch consume path. `sessionOpeningDjRef` stays true until opener synthesis completes and `play()` is called or fails. Lookahead MUST NOT start while the current gap break is still speaking (`introRunningRef` and music not yet released). After `startSongAtFullVolume`, two-ahead may arm. `DjBreakPrefetchEngine.has()` / `take()` report only a completed `audioBuffer` in `prefetchedBreaksMap` — in-flight TTS is not a warmed hit.
...
**Copy cadence (pace → opener):** Host Studio pace, talk level, knowledge, and lore tier are forwarded explicitly into every live generate-script POST (`AudioPlayer` → `liveHostStudioScriptFields` / `dj-intro.hostStudioScriptFields`). Do not infer Every Song from stale localStorage chatter when the UI says Every Song. Every Song mid-session lore must **teach first** and must **not** inject `previousTrack` as “You just heard / That was” recap bait. The announcement clip leads with `up next` / `now playing` / `here's`. Natural Pace and Long Breaks **may** open with “You just heard / That was” when bridging after silent songs. Pass 4 first-playlist Song 1 → Song 2 pack stays the one-shot “That was … Up next …” pair. Catch-up `recap` stays lookback. Talkative: at most one SonGhost identity beat per break (stinger/brief owns it — lore must not also ID). `promptBuilder.buildCopyCadenceDirective` / `paceGuidance` encode the opener rule.
...
#### Prefetch lookahead (two-ahead from song start)

Pass 3: `twoAheadTargets` + `ensureTwoAhead` / AudioPlayer `tryArmLookahead` start warmup when the song starts at 100% (or the queue is known). Keep **two** finished packages (N+1, N+2). Cache key = upcoming track id + settings fingerprint (voice, lore tier, persona, vibe, pacing, city, explicit, station). N+2 is planned from N+1's `nextState` — do not re-roll Natural Pace.

Local Custom: serialize TTS (one GPU job; second waits). OpenAI: may prefetch both in parallel without blocking the local slot. Skip / station / Host Studio script-or-voice settings: abort + drop + retarget immediately. Pass 1 gates still refuse late play over music. Missing package at the cut: Pass 2 fallback if `canPlay`, else skip → song at 100%.
...
- `normalizeTrackRefs` / `parseLoreTrackRefs` keep the newest N entries via `.slice(-limit)` (chronological buffers, newest last) so a long history cannot surface a ~4-songs-ago title as "what we just heard". Secondary `recentHistory` rows are older background context only — when lookback copy is used (Natural Pace / Long Breaks / first-playlist pack / catch-up recap), "That was [Song]..." MUST name `previousTrack` alone. Every Song mid-session does not default to that lookback opener.
...
**Two-ahead from song start:** Live dial and companion progress arm N+1 and N+2 immediately when the song is on. Local GPU jobs serialize (`skipIfBusy` queues N+2). OpenAI may run both prefetch jobs. A late clip must never play after music has started (Pass 1). Near-end `getPrefetchLeadSeconds` is a fallback only.
...
**Skip abort:** Live dial `AudioPlayer.skipNext` MUST abort the current break-flight generation, cancel in-flight `/api/generate-voice` + sidecar work, and drop stale cached clips **before** advancing. `page.tsx` `skipTrack` MUST invoke `abortPendingSpeechAndClearBuffers` (sessionEpoch bump + prefetch controller cancel) before DirectStream skip (quarantined: `spotifyRemote.next()` / `previous()`) so in-flight TTS cannot air on the skipped-to track. The **60-minute sliding skip limiter** still gates whether the skip is issued.

---

## 4. TTS Synthesis Pipeline

Script generation (`/api/generate-script`) and shared prep (`src/lib/tts.ts`) produce the spoken payload. The **live dial** TTS provider comes from Host Studio `preferredVoice` (`resolvePreferredVoiceTarget` / `resolveLiveHost`): an OpenAI voice id synthesizes on **`gpt-4o-mini-tts`** (`OPENAI_TTS_MODEL`) with persona `ttsInstructions`; a `local:N` pick sends `provider: "local"` + `voiceSlot` to `/api/generate-voice`. Prefetch warmup stamps the same provider/voice/slot so a warmed clip cannot be OpenAI then play local (or the reverse). Explicit `provider: "local"` returns **real Chatterbox-Turbo speech bytes** (`audio/wav`) from the `LOCAL_TTS_URL` sidecar (same blob contract as OpenAI — VoiceNode already plays any blob). GPU or model unavailable **fails closed** (no beep, no OpenAI fallback; the break is skipped). Live YouTube remains **gap-then-100%**: host speaks in the pre-song gap; the song starts at full volume after the last clip — no ducking / talk-over, including when the clip came from `"local"`. **Never queue a late play.** Each break attempt has a generation token (`BreakFlightCoordinator`) minted before any TTS await. Skip, new station/playlist, and Host Studio settings that affect script/voice abort the in-flight fetch and sidecar job. If the voice package is not ready by **20s** (`VOICE_PACKAGE_DEADLINE_MS`), skip the live break; a Custom host may play a prerecorded fallback in the gap, then the song starts at 100%. `VoiceNode.play()` MUST refuse when music for that transition has already started or the generation is stale. If `LOCAL_TTS_URL` is unset or the sidecar errors, generate-voice **fails closed** (HTTP error, no OpenAI fallback). Local sidecar HTTP is aborted at **28s**; the sidecar also accepts `POST /v1/cancel` and treats client disconnect as abort (discard WAV; CUDA forward is not preemptable, so the next job waits for that forward). Sidecar rejects text over **520** characters, caches per-slot speaker conditionals, and logs `duration_ms` plus abort reason. Chatterbox-Turbo has no extra inference-step knob (already 1-step decoder). Reference WAVs should be **6–12s** clean reads. ElevenLabs (`eleven_turbo_v2_5`) remains in-tree for explicit `provider: "elevenlabs"` callers (WS-7 Director's Cut) and is mothballed from Host Studio. Downstream engines receive only sanitized plain text. Input longer than **2000** characters is rejected, not truncated (`assertOpenAiTtsInputLength` / `OPENAI_TTS_MAX_INPUT_CHARS`).

### 4.0a Prerecorded Custom banks (Pass 2)

Disk clips so Custom hosts can greet and recover without a live sidecar job at the transition.

| Bank | When it fires | When it must not fire |
|------|----------------|------------------------|
...
While Track N plays, keep two **finished** packages ready for the next two transitions. Fetch starts at song start (`startSongAtFullVolume` / first progress tick), not inside the old 30–120s lead window.

| Rule | Behavior |
|------|----------|
| Depth | Exactly two upcoming tracks (`TWO_AHEAD_DEPTH`). No third lookahead. |
| Cache key | Track id + settings fingerprint. Voice / lore / persona / vibe / pacing / city / explicit / station change → miss + rebuild. |
| Local GPU | One synth at a time. N+2 waits in FIFO until N+1 finishes. |
| OpenAI | Up to two prefetch jobs in parallel. Does not occupy the local slot. |
| Skip / station / settings | Pass 1 abort + drop stale + immediately retarget the new N+1 / N+2. |
| Consume | Prefer the prebuilt package. If missing and still in the gap: Pass 2 fallback (Custom). If music already started: Pass 1 refuse-late-play. |
| Pass 4 | First-playlist "that was / up next" after Song 1 of a new listen only. Mid-session cadence unchanged. Hook: `planDjSegment({ isFirstPlaylistTransition })` from `AudioPlayer.tryArmLookahead`. |
...
**Host identity lock (MUST):** The persisted / UI host id is resolved through `resolvePersonaId()` before TTS. `LEGACY_PERSONA_ALIASES` maps old named-host ids (e.g. `"devon"` / `"devon-pulse"` → `"warm-companion"`, `"jasper-reed"` → `"the-musicologist"`) plus job-name slugs / old labels (`"the-guide"` / `"Warm Companion"` → The Guide, `"the-critic"` → The Critic, `"the-archivist"` → The Archivist) and MUST NOT collapse an unknown id to a different live persona when a valid alias exists. `DEFAULT_PERSONA` is **Standard Broadcast**. Voice is a separate axis: `preferredVoice` is never rewritten by persona migration (`LEGACY_PERSONA_VOICE` is fallback only). TTS synthesis MUST use the listener's stored OpenAI voice plus the persona's `ttsInstructions` — a locked Guide (`warm-companion`) cannot air a different job's delivery while the UI still shows The Guide.

**Persona = host job (MUST):** Lore tier owns depth; pace owns how often the host talks; persona owns *which teacher* is in the booth. The Guide must land one ear cue (`listen for` / `when the next track hits, notice`). The Critic must land one judgment plus a craft reason. The Archivist must land one lineage / scene / city / label / technique-through-time link. Shared contract: hook → payoff → setup/handoff; `cleanTrackForSpeech` names only; SongHost = DJ, SonGhost = station; no empty hype (`vibes`, `journey`, `timeless`, `iconic` unless tied to craft, `weekend adventurers`). Every Song stays tight; Director's Cut owns length. Blind test: from the transcript alone a listener should usually guess Guide vs Critic vs Archivist.

**Persona quality gate (MUST):** After script generation, `runPersonaScriptQualityGate` may trigger **one** retry if a teaching clip misses the required-move regex (Guide without listen/notice, Critic without judgment+craft, Archivist without lineage), uses banned filler as empty hype, or trips the clone-opener heuristic. A miss of the required move is a failure even when other heuristics pass; after retry, air the attempt that landed the move (or the better score) — never hang the gap, never talk over music. Local TTS remains fail-closed. Do not validate session-opening templates, `song_id`, stingers, recaps, or names-only announcement clips.
...
3. Station initialization / default-station loading on mount MUST check **`isHostLocked || savedHostId`** (`shouldRetainHost()`) **before** applying `station.defaultPersonaId` / `defaultHostId`. A restored host id **MUST take priority** over curated station defaults so a refresh cannot silently replace The Archivist (or any locked pick) with the station's default DJ.
