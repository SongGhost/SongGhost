...
| `src/components/AudioPlayer.tsx` | Live YouTube / DirectStream path: queue + scheduler + VoiceNode + prefetch. `suppressLocalAudio` is hardcoded `false`. `handleNewTrack` ignores `companionActiveRef` while `isDirectStreamMode` is active. When `sessionOpeningDjRef` is true, it arms `setLaunchHold(true, "hard_pause")` **synchronously** before any `await` and skips `resolveLocalEvent` for station-launch liners. It MUST pause the live embed, speak with `duckMusic: false`, then start the song at `UNDUCKED_GAIN`. Never pass `duckingTarget`. If local TTS is not ready before music release, abort the in-flight voice and do not call `play()` late. `releaseLaunchDuck` fail-closes skipped / failed openers over 600 ms; a 3s playhead watchdog restores to `1.0` when `voiceNode.isSpeaking()` is false. VoiceNode `onEnded` / `finishDjSegment`, `releaseOpenerHold`, and the restore watchdog MUST call `provider.releaseLaunchHold()`. Pavlovian `onLoreComplete` fires after lore + optional sweeper + announcement. `performanceCommit` stays armed for licensed DirectStream plays **>30s**. Next is disabled when `skip-limiter` is exhausted. T39 intercepts (`justSkippedRef`, `restoreRampEndsAtRef`, 8s stall watchdog) live here — see AUDIO_ORCHESTRATION_SPEC_2. |
...
| `src/lib/audio/dj-prefetch.ts` | Unified lookahead (`LOOKAHEAD_SECONDS = PREFETCH_LOOKAHEAD_SECONDS`, default 30s). `shouldStartLookahead` accepts `leadSeconds` so local Director's Cut can warm at 120s. `skipIfBusy` skips a second local GPU synth. |
| `src/lib/dj/prefetchEngine.ts` | Zero-latency warmup (`PREFETCH_LOOKAHEAD_SECONDS = 30`; Time Capsule 45s; Director's Cut 60s). Local Chatterbox uses 75s / 100s / 120s via `getPrefetchLeadSeconds(format, provider)`. Overlapping local synths are skipped, not aborted onto a dead socket. |
...
`LOOKAHEAD_SECONDS = PREFETCH_LOOKAHEAD_SECONDS = 30` in `src/lib/audio/dj-prefetch.ts` (re-export of the unified OpenAI default). Live AudioPlayer passes `getPrefetchLeadSeconds(commentaryFormat, provider)` so local Custom voices warm earlier (75s default, 100s Time Capsule, 120s Director's Cut).
...
5. `sessionOpeningDjRef` stays true until opener synthesis completes and `play()` is called or fails. 30s lookahead also gates on `!introRunningRef.current`.
