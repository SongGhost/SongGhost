...
|---------|-----------------|-----------------------------------|
| **YouTube IFrame API** | Live dial music bus. Default host is off-screen `fixed -left-[9999px]` at 320×180. Test-only **YT View** toggle (`DevTierBadge`, `songhost_youtube_viewer`) surfaces the same iframe in the dock at 320×200 via `setSize` — no remount. | `useYouTubePlayer.ts`, `YouTubeTrackProvider` (`TrackProvider.ts`), host in `AudioPlayer` |
