...
| `src/lib/audio/prerecorded.ts` | Custom-host disk banks (Pass 2). 16 welcomes + 16 fallbacks per slot (`local:1`–`4` = Harris/Piper/Quinn/Bea). Scripts: `src/lib/audio/prerecorded/scripts.json`. WAVs under `public/audio/prerecorded/slot-{N}/` after `npx tsx tools/local-tts-sidecar/generate-prerecorded.ts`. Welcome on session start; fallback on Pass 1 skip/timeout/not-ready **only while the gap is open**. OpenAI voices skip this bank. |
