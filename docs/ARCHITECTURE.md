...
**Segment-kind routing (not a commentary format):** `song_intro` is single-clip with **no earcon** (opener = rotated station liner; mid-session = "up now is [song] by [artist]"). Pavlovian two-clip (earcon → lore → ducked announcement) is `artist_trivia` and `local_events` only. Earcons fire only for `artist_trivia`, `local_events` (weather or concert), and `roots_teaser`.
