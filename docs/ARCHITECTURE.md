...
**Live-LLM lore:** Lore clips are generated live by the LLM under `STRICT_TRUTH_GUARDRAIL` (no invented biography/recording anecdotes; vibe/production/chart context only). **Commentary styles** (`COMMENTARY_STYLES`, 7 on-air angles) rotate per break via `pickCommentaryStyle(styleRotationIndex)` for all lore kinds; the rotation is **city-aware** — `weather_vibe` and `local_events` styles are excluded when no broadcast city is known.
