...
Track 1 session opener (MUST — pre-ducked `intro_ramp` from `0:00`, not an un-held start):

1. `stationId` / `queueGeneration` arms `launchHoldActive` as `intro_ramp`, pins `duckBus` to `DUCK_RATIO`, and calls `setLaunchHold(true, "intro_ramp")` before play/load effects.
2. `handleNewTrack` (while `sessionOpeningDjRef`) re-arms the hold **before any `await`**. `resolveStationLaunchHoldMode` is the sole opener-mode authority: missing / unprobed intros stay `intro_ramp`; `hard_pause` only when `introDurationSec` is confirmed and `< 3`. `shouldPauseForStationLaunchVocals` MUST NOT force `hard_pause` over `intro_ramp`.
3. `intro_ramp`: element plays from `0:00` already at `DUCK_RATIO = 0.18`. `hard_pause`: element stays paused at `0:00` (confirmed cold vocal intros only). `intro_ramp` MUST NOT `pause()` or `seekTo(0)`.
...
| Song Radio | `song-radio-` | Seed stays at index 0; tail is a Last.fm / MusicBrainz similar-artist mix (`applyArtistCap` max 2), matching Artist Radio. Same-artist catalog fallback is forbidden. Replenish via `/api/recommendations` using `station.seedArtists` when `spotifyId` is missing. Launch rows stamp `youtubeId: ""` plus an HTTP `previewUrl` / `streamUrl` so `resolveDirectStreamUrl` attaches `DirectStreamProvider` (native HTML5 `<audio>`); the YouTube IFrame player stays unmounted for Pocket Mode. |
...
| `/api/song-radio` | GET | Song Radio catalog build. Seed identity MUST pass `itunesTitlesMatch` + `itunesArtistsMatch` before preview bind (`catalogPreviewUrl` / `attachSeedCatalog`). Catalog pin is `lookupITunesSongById` then `lookupITunesTrack` — never rank-0. `resolveCandidate` MUST NOT resolve or stamp YouTube video IDs (`youtubeId` is always `""`); HTTP media comes from Spotify `preview_url` or iTunes `previewUrl`, and candidates with neither `streamUrl` nor `previewUrl` are dropped so search launches route through `DirectStreamProvider` (native HTML5 `<audio>`) for mobile Pocket Mode. Requested seed stays at index 0; the tail is a Last.fm / MusicBrainz similar-artist mix (`fetchSimilarArtists`, same source as `/api/artist-radio`) with `applyArtistCap` (max 2 per act). Same-artist iTunes fallback is forbidden. An empty similar-artist pool or a payload with fewer than 2 unique primary artists returns **404**. Historical Spotify recommendations are optional when an app token is present. `createSongRadioStation` stamps `seedArtists: [artist]`. |
