...
| Music transport (primary) | **`DirectStreamProvider`** — un-suppressed HTML5 `<audio>`; mix-bus `musicGain()` on the element; single `captureMediaElement` analyser tap; Track-1 `launchHoldActive` (`hard_pause` on the live dial). `AudioPlayer` hardcodes `suppressLocalAudio = false`. Binding: `useDirectStreamPlayer`. **Single-transport isolation:** `isPlaying` is gated to the one active on-air hook (DirectStream XOR preview XOR YouTube). `duckBus.setVolume()` applies `setDuckGain` only to that provider — never a fan-out to idle transports. **Live dial:** station select arms `hard_pause` and MUST NOT pin `duckBus` to `DUCK_RATIO`. Host speaks in the pre-song gap; song starts at `UNDUCKED_GAIN`. If an opener liner is skipped, `releaseLaunchDuck` restores to `UNDUCKED_GAIN` over 600 ms; a 3-second playhead watchdog (`currentTime > 3` and `voiceNode.isSpeaking() === false`) is the fallback. |
...
| Speech & AI | **OpenAI GPT-4o-mini** (scripts / curator), **OpenAI `gpt-4o-mini-tts`** (default live-dial TTS, all 13 voices + `instructions` steerable prosody; 2000-char cap via `assertOpenAiTtsInputLength`). **`local`** is a typed `TtsProvider` / `VoiceProviderId` that proxies `/api/generate-voice` to a `LOCAL_TTS_URL` sidecar (`tools/local-tts-sidecar/`; **Phase B Chatterbox-Turbo on the laptop GPU**, fail-closed if CUDA/model unavailable). **WS-8 Phase C:** Host Studio lists the 13 OpenAI voices plus 4 laptop custom slots (`preferredVoice` = OpenAI id or `local:1` … `local:4`). Custom audition hits `/api/studio/voice-preview` → sidecar and **fails closed** (no silent OpenAI sample). Sidecar down or URL unset **fails closed** — no OpenAI fallback. **WS-8 Phase D:** live dial follows the Host Studio pick (`resolvePreferredVoiceTarget` / `resolveLiveHost`): OpenAI voice id → `ttsProvider: "openai"`; `local:N` → `"local"` + `voiceSlot`. Custom selected + sidecar down **fails closed** (skip the break; no silent OpenAI substitute). **ElevenLabs** REST (`eleven_turbo_v2_5`) is mothballed from the live dial; kept in-tree for admin-only WS-7 Director's Cut pre-renders. **Cartesia** is typed (`VoiceProviderId`) but not wired. Personas: 4 character ids in `src/data/personas.ts`. `LEGACY_PERSONA_ALIASES` maps old named-host ids → live personas; `LEGACY_PERSONA_VOICE` is fallback voice only — stored `preferredVoice` is never rewritten. |
...
flowchart TB
  ...
  subgraph Engine["SongHost Audio / Player"]
    ...
    Prefetch["lib/audio/dj-prefetch (two-ahead)"]
    Prefetch30["lib/dj/prefetchEngine (two-ahead)"]
...
7. **Fail-closed station-launch restore** — `stationId` / `queueGeneration` arms `launchHoldActive` / `sessionOpeningDjRef` as `hard_pause` and MUST NOT pin `duckBus` to `DUCK_RATIO` (18%). Live AudioPlayer never passes `duckingTarget`. If the opener liner fails, returns null, is skipped (`silent` / `music_only` / persona lock), or `handleNewTrack` early-returns before speech, `releaseLaunchDuck` ramps `duckBus` to `UNDUCKED_GAIN` over `STATION_LAUNCH_RESTORE_MS` (600 ms). Safety watchdog: if playhead `currentTime > 3.0` and `voiceNode.isSpeaking()` is false, restore `duckBus` to `1.0`.
...
├── hooks/
│   ├── useStationQueue.ts       # Infinite statutory queue, replenish, recentTrackIds, two-ahead DJ prefetch, §114 admitStatutory
...
| `src/components/AudioPlayer.tsx` | Live YouTube / DirectStream path: queue + scheduler + VoiceNode + prefetch. After a new station/playlist welcome, Song 1 → Song 2 uses a one-shot first-playlist “that was / up next” pack (Pass 4); later two-ahead stays on existing cadence. `suppressLocalAudio` is hardcoded `false`. `handleNewTrack` ignores `companionActiveRef` while `isDirectStreamMode` is active. When `sessionOpeningDjRef` is true, it arms `setLaunchHold(true, "hard_pause")` **synchronously** before any `await` and skips `resolveLocalEvent` for station-launch liners. It MUST pause the live embed, speak with `duckMusic: false`, then start the song at `UNDUCKED_GAIN`. Never pass `duckingTarget`. Each break attempt owns a **generation token** (`BreakFlightCoordinator`) minted **before** any TTS await. Skip, station/playlist change, and Host Studio settings that affect script/voice abort in-flight `/api/generate-voice` (and the sidecar job) and drop stale cached work. If the voice package is not on-air within **20s** (`VOICE_PACKAGE_DEADLINE_MS`), skip the live break; when a Custom host (`local:1`–`4`) is selected and the gap is still open, play a short **prerecorded fallback** then start the song at 100% — never queue a late play. `VoiceNode.play()` MUST refuse when music for that transition has already started or the generation is stale. **Session start + Custom:** play a random **prerecorded welcome** from `/audio/prerecorded/slot-{N}/` (scripts in `src/lib/audio/prerecorded/scripts.json`) in the pre-song gap, then Song 1 at 100%. OpenAI voices skip the disk bank (live launch liner stays). WAVs are **committed** under `public/audio/prerecorded/` and served statically on Vercel. Regenerate locally only with `npm run generate-prerecorded` (sidecar Ready) — never at Vercel build. If even the fallback WAV is missing, fail closed (silence; no OpenAI substitute). `releaseLaunchDuck` fail-closes skipped / failed openers over 600 ms; a 3s playhead watchdog restores to `1.0` when `voiceNode.isSpeaking()` is false. VoiceNode `onEnded` / `finishDjSegment`, `releaseOpenerHold`, and the restore watchdog MUST call `provider.releaseLaunchHold()`. Pavlovian `onLoreComplete` fires after lore + optional sweeper + announcement. `performanceCommit` stays armed for licensed DirectStream plays **>30s**. Next is disabled when `skip-limiter` is exhausted. T39 intercepts (`justSkippedRef`, `restoreRampEndsAtRef`, 8s stall watchdog) live here — see AUDIO_ORCHESTRATION_SPEC_2. |
...
| `src/lib/audio/prerecorded.ts` | Custom-host disk banks (Pass 2). 16 welcomes + 16 fallbacks per slot (`local:1`–`4` = Harris/Piper/Quinn/Bea). Scripts: `src/lib/audio/prerecorded/scripts.json`. Checked-in WAVs at `public/audio/prerecorded/slot-{N}/{welcome\|fallback}-NN.wav`, served as `/audio/prerecorded/slot-{N}/…` (manifest: `public/audio/prerecorded/manifest.json`). Regenerate locally only (`npm run generate-prerecorded`; sidecar Ready) — never at Vercel build. Welcome on session start; fallback on Pass 1 skip/timeout/not-ready **only while the gap is open**. Missing WAV / missing bank fail closed (welcome → live liner; fallback → silence). OpenAI voices skip this bank. |
...
| `src/lib/audio/dj-prefetch.ts` | Two-ahead lookahead from song start. Plans N+1 then N+2 (N+2 from N+1 `nextState`). `skipIfBusy` queues a second local GPU job; OpenAI may run both in parallel. Near-end `shouldStartLookahead` remains as a late helper. |
| `src/lib/dj/prefetchEngine.ts` | Two-ahead warmup into `prefetchedBreaksMap` keyed by track id + settings fingerprint (`src/lib/dj/breakPackageCache.ts`). Local GPU: one synth at a time, FIFO queue. OpenAI: up to two parallel prefetch jobs (does not occupy the local slot). Lead-window helpers remain as a late fallback. No Director's Cut length soft-cap. |
| `src/lib/dj/breakPackageCache.ts` | Cache key + `twoAheadTargets()` (exactly two upcoming transitions). Fingerprint covers voice, lore tier, persona, vibe, pacing, city, explicit, station. First-playlist "that was / up next" is planned in `planDjSegment({ isFirstPlaylistTransition })`, not in this helper. |
...
### Host vs music (`mix-bus.ts` + `VoiceNode` / `dj-intro.ts`)

**Live YouTube dial (MUST):** do not start a song until the DJ is done speaking, then start at **100%**. `playDjIntro` defaults `duckMusic` to false. AudioPlayer never passes `duckingTarget`. Mix-bus duck constants below remain for quarantined companion, VoiceNode tests, and fail-closed restores.

| Parameter | Constant | Value |
|-----------|----------|-------|
| Duck target (unused on live dial) | `DUCK_RATIO` | **18%** of master |
| Duck ramp in (unused on live dial) | `DUCK_RAMP_MS` | **300 ms** |
| Restore ramp out (fail-closed / VoiceNode) | `RESTORE_RAMP_MS` | **1500 ms** |
| Track-1 fail-closed restore | `STATION_LAUNCH_RESTORE_MS` | **600 ms** (`scriptGenerator.ts`) |
...
**Live YouTube / DirectStream AudioPlayer** pauses for every voiced break and starts the song at `UNDUCKED_GAIN`. Format-aware Pause–Talk–Resume on DirectStream is Phase 6. Quarantined companion stays duration-based Mode A/B.

### Pre-fetch sequence A — DirectStream / AudioPlayer (two-ahead from song start)

When the song starts at 100% (`startSongAtFullVolume`) or the queue is known, AudioPlayer arms the next **two** transitions. N+1 is planned from the live scheduler; N+2 is planned from N+1's `nextState` so Natural Pace / non–Every Song cadence is not redesigned. Local Custom: one GPU synth at a time (N+2 queues). OpenAI: both jobs may run in parallel.

```text
song start → twoAheadTargets(queue, index)  // N+1 and N+2
  → planDjSegment(N+1) then planDjSegment(N+2 from N+1.nextState)
      // First listen only: N+1 after Song 1 is the that-was / up-next pack
  → DjPrefetchController.start(each key)
      → generateDjBreak / generatePavlovianDjBreak
      → VoiceNode.preload(blob)  (muted + volume 0 before .src; never session AudioContext)
  → on transition: take(trackKey) → play warmed blob
  → miss in gap: Pass 2 prerecorded fallback (Custom) if canPlay; else clean skip → song at 100%
```

**First-playlist special pack (Pass 4):** After the session welcome / opener, Song 1 plays at 100%. While it plays, two-ahead warms a dedicated pack for the Song 1 → Song 2 gap: lore recap (“That was [Song 1]…”) then an up-next tease (“Up next [Song 2]…”) using the live Host Studio lore tier / persona / voice. On Director's Cut the that-was clip must teach (craft/history **and** why it matters), then a new capitalized sentence — never “Clan. recorded”. Song 2 then starts at 100%. This pack is **one-shot** — skip during Song 1, station change, or Host Studio script/voice edits abort it (Pass 1). Later mid-session transitions: lore clips teach first; the announcement clip may lead with what's next. Natural Pace / Long Breaks may still open with “You just heard / That was” after silent songs. Never talk over music.

Skip / new station-playlist / Host Studio settings that change script or voice: abort in-flight (Pass 1), drop stale packages, immediately retarget the new two upcoming tracks. Pass 1 refuse-late-play still wins — never play late audio over music.
...
### Pre-fetch sequence B — Zero-latency engine (two-ahead)

`useStationQueue.notePlaybackProgress` calls `ensureTwoAhead` from song start (companion path). YouTube / DirectStream set `skipEnginePrefetch` so AudioPlayer owns TTS (no double-synth). Near-end `shouldPrefetchUpcomingBreak` remains as a late fallback. There is **no** exported `COMPANION_PREFETCH_NEAR_END_MS`.

```text
song start / queue known
  → ensureTwoAhead([N+1, N+2])
      → /api/generate-script  then  /api/generate-voice
      → cache ArrayBuffer + Blob in prefetchedBreaksMap  // key = fingerprint + track id
  → on transition: take(trackKey) if fingerprint matches
```

Rules:

- Keep **two** finished packages ahead. Local GPU: one synth job; the second waits in FIFO. OpenAI may prefetch both without occupying the local slot.
...
**Live dial — pause, then start at 100%**

**Lore-type breaks** (`isLoreSegmentKind`: mid-session `song_intro`, `up_next`, `artist_trivia`, `local_events`) use the **Pavlovian two-clip** sequence (WS-6): earcon → ~500 ms commentary gap (`COMMENTARY_GAP_MS`) → live-LLM lore clip in silence → optional station-ID sweeper (`plan.includeStinger`) → announcement in silence → `onLoreComplete` starts Track B at **100%**. **Session-opening `song_intro` is single-clip** (no earcon, no lore, no LLM, no 500 ms gap): a templated station liner (`getStationLaunchClips().line`) in the pre-song gap, then the song at 100%. **`song_id`** is a names-only kind (artist + song, varied phrasing) for the "always tell me what's playing" announce — never lore, never Pavlovian: announce in the pre-song gap, then start at full volume. Stinger and recap stay single-clip in the gap, with no earcon. A standalone `stinger` track uses the single-clip sweeper, so a track never gets both a standalone stinger and a Pavlovian sweeper. **Free Roots & Branches teaser** (`kind: "roots_teaser"`, WS-4) is also single-clip (not Pavlovian): `teaser/open.mp3` earcon → ~500 ms gap → one short script in the pre-song gap. It is excluded from `isLoreSegmentKind()`. Canonical FSM: [AUDIO_ORCHESTRATION_SPEC_2.md](./AUDIO_ORCHESTRATION_SPEC_2.md) §1. Quarantined companion `runPavlovianTransition` may still duck; it is not the live dial.

Mid-session single-clip (stinger / recap / `song_id`; Track 2+):

1. Music keeps playing at full gain through prefetch / live TTS (`PREFETCHING_BREAK`).
2. `handleNewTrack` pauses the live embed. `playDjIntro` uses `duckMusic: false` (no `duckingTarget`).
3. Prefetched (or live) DJ clip plays at `voiceGain`.
4. On speech end, `startSongAtFullVolume()` / `onBreakExit` starts the song at `UNDUCKED_GAIN`. Fail / timeout / abort MUST also start at 100%, never leave the embed paused.

Track 1 session opener (MUST — transport hold from `0:00`):

1. `stationId` / `queueGeneration` arms `launchHoldActive` as `hard_pause` and calls `setLaunchHold(true, "hard_pause")` before play/load effects. It MUST NOT pin `duckBus` to `DUCK_RATIO`. `DirectStreamProvider.setLaunchHold` does **not** pin duck gain. `armStationHandoff()` is armed only when a live companion/Spotify session will own the opener.
2. `handleNewTrack` (while `sessionOpeningDjRef`) re-arms the hold **before any `await`**. Seek to `0` and pause the YouTube / preview embed. Never pass `duckingTarget`.
3. Station-launch liners skip `resolveLocalEvent` so location fetch cannot delay TTS.
4. `releaseOpenerHold`: MUST call `provider.releaseLaunchHold()`, set `duckBus` to `UNDUCKED_GAIN`, and play. If the playhead leaked past 1s, do not rewind. If `introRunningRef` remains true past `speechDuration + RESTORE_RAMP_MS + 1500 ms`, force that start so Track 2 is not blocked. Never toggles React `isPlaying`.
5. `sessionOpeningDjRef` stays true until opener synthesis completes and `play()` is called or fails. Two-ahead lookahead also gates while the current gap break is speaking (music not yet released).
6. **Fail-closed restore:** `releaseLaunchDuck(reason)` ramps `duckBus` to `UNDUCKED_GAIN` over 600 ms on every opener skip / null TTS / companion-local early-return. Safety watchdog: if `currentTime > 3.0` and `voiceNode.isSpeaking()` is false, restore `duckBus` to `1.0`.
...
**Extended formats** (`roots_branches`, `time_capsule`, `directors_cut`) — Pause–Talk–Resume is **Phase 6** on the live DirectStream bus. Live `AudioPlayer` never ducks; it speaks in the gap then starts at 100%. It does **not** call `resolveBreakTransitionPolicy`. That helper (`EXTENDED_BREAK_AMBIENT_FLOOR = 0.05`, `pause_or_ambient`) is consumed by quarantined companion (`webOrchestrator.ts`). Do not document companion or DirectStream Pause–Talk–Resume as live.
...
Pause until audio unlock → arm `launchHoldActive` as `hard_pause` → keep Track 1 paused at **0** until the opener liner finishes → start at `UNDUCKED_GAIN`. Emit on-playing **once per track load** (no `onPaused` bounce) so `handleNewTrack` can arm the liner, then pause again. `setLaunchHold` does not modify duck gain. Station select MUST NOT pin `duckBus` to `DUCK_RATIO`. Position ticks MUST NOT re-pin `setDuckGain(DUCK_RATIO)`. Playhead `> 3s` while playing auto-releases the hold flag; if `voiceNode.isSpeaking()` is still false, `releaseLaunchDuck` restores `duckBus` to `1.0`. Track 1 MUST NOT start under the opener. Binding: `useDirectStreamPlayer` exposes `setLaunchHold`, `releaseLaunchHold`, `isLaunchHoldActive`, `getLaunchHoldActive`, `getLaunchHoldMode`.

### YouTube first-song invariant (`useYouTubePlayer.ts`) — current dial transport

Pause until audio unlock → single `seekTo(0)` → play → `tryEmitOnPlaying()` **once per track load** → AudioPlayer pauses for the opener liner → song starts at 100% after speech. Never `setVolume(18)` on the iframe for a DJ break. This is the live dial path today. Host is always visible in-flow at 320×200 (no off-screen hide; `setSize` via `viewerVisible: true`).
...
- **`preferredVoice`** — listener's Host Studio voice (`PreferredVoice`: an OpenAI `VoiceOption` or `local:1` … `local:4`; never Pro-gated). Independent of persona. `resolvePreferredVoiceTarget()` / `resolveLiveHost()` map a custom id to `provider: "local"` + slot. Live dial uses that pick for between-song breaks and the session opener. Migration never rewrites this field (`LEGACY_PERSONA_VOICE` is fallback only when a caller has a legacy host id and no stored voice).
...
- **`commentaryFormat`** — lore / commentary depth (Phase 7). Defaults to `"standard"`. Persisted via `setCommentaryFormat()`; Host Settings Drawer exposes "Lore & Commentary Depth" (`CommentaryFormatSelector` in `HostBar.tsx`). UI display labels are standardized across the HostBar summary pill (`formatCommentaryFormatLabel` / `formatHostSettingsSummary` in `HostBar.tsx`) and the Host Settings Drawer: **`"Standard"`**, **`"Roots & Branches"`**, **`"Sonic Time Capsule"`**, and **`"Director's Cut"`**. The Host Studio pill subscribes to live `UserPreferences.commentaryFormat` so a drawer change updates immediately (e.g. `Natural Pace • Director's Cut`). Extended values `roots_branches`, `time_capsule`, and `directors_cut` are Pro-gated and append format + SSML pacing directives in `promptBuilder.buildCommentaryFormatDirective()`. **Lore tier is an insight-volume knob, not a label:** mid-session Director's Cut lore is **80–110 words / ~35–45s** and must teach — **one concrete craft/history beat** (studio, producer, technique, or scene) **and** one **why-it-matters** beat, then a clean handoff. It must not collapse to a title-only “Up now is …” line. Sonic Time Capsule **55–75 words / ~25s**; Roots & Branches **25–32 words / ~12–14s**; Standard stays a short clean intro. The live YouTube path must not override Director's Cut with the old 1-fact / 16–20-word / 10s Standard cap. **Local Chatterbox Director's Cut is not soft-capped** — same 80–110 teaching contract as OpenAI (`src/lib/dj/loreBudget.ts`). Time Capsule may still use a tighter local GPU budget (**38–48**); Roots **25–32**; Standard **15–22**. Prefetch lead (local DC **120s**), the **20s** gap deadline, and never-talk-over stay. After a period, start a new capitalized sentence (never “Clan. recorded”). **`Long Breaks` is frequency only** (rarer breaks) — it does not substitute for Director's Cut depth. The live dial (`dj-intro.ts`) forwards Host Studio talk/chatter pacing, pace, `allowExplicit`, and knowledge on `/api/generate-script` POSTs so server defaults do not fight the UI. **Spoken brand:** the host refers to themselves as **SongHost**; station identity / "you're listening to" lines use **SonGhost** (never SongGhost, never "Song Ghost"). URL is songhost.com. **`clampHostTuningForTier` still forces Free `lore: "standard"`** — Free listeners never get the full `roots_branches` format. WS-4 adds a separate Free-only `DjSegmentKind` `"roots_teaser"` (not a `commentaryFormat` value): every 7th voiced break is a short Roots & Branches preview (teaser earcon + 14–18 word taste + in-character Pro sign-off + vernacular outro + inline "Pro Preview" badge). Pro never hears the teaser. Mid-session upgrade clears `teaserSlotCount` and drops any pending teaser. DirectStream (and the quarantined Spotify companion `useWebOrchestrator` path) folds this through `resolveStationSettings()` (station override > global) rather than reading the raw global preference. Host Settings snaps Free-tier selections back to `"standard"`.
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/generate-script` | POST | LLM DJ script (+ optional lore cache / embedded TTS audio URL). **Live YouTube uses the legacy assembler** (`handleLegacyScriptGeneration` via `buildDjScriptPrompt` + `assembleLegacySystemPrompt`) because `AudioPlayer` / `dj-intro` POST **without `trackId`** — that path is **not** the lore-cache assembler (`isLoreCacheRequest`). Mid-session teaching lore gets **one contract**: persona required move + lore-tier budget/depth + pace copy-cadence. Higher lore tiers do not inherit Standard “title/artist/station ID”, 12-word-everything, or one-nugget caps. Teaching truth prefers verified craft **or** a general technique without fake proper nouns (no “describe the vibe” fluff escape). Rotating commentary style / musicology pillar may colour topic only — they must not erase Guide ear-cue, Critic judgment+craft, or Archivist lineage. Talkative lore clips must not also demand a station ID. There is **no** positive DB fact-grounding / no fact injection — anti-repetition only (`getExcludedFactTopics` + `recentBreakHistory`). Free-tier quota: `403 QUOTA_EXCEEDED` when `breakCount >= 30`; increments meter after successful new generation. Free-tier pace guard forces `djMode: "balanced"` / `talkLevel: "standard"` (`breakPace: "short"`). Lore-type breaks accept `scriptPhase` of lore, announcement, or full (Pavlovian two-clip; `full` is the legacy single-clip brief). Live dial forwards explicit `pace`, `talkLevel`, `knowledge`, and lore tier from Host Studio. Announcement clip is track name + artist only. `buildLoreSystemPrompt` injects `buildVibeDirective()` so Host Studio notes apply on DirectStream (and on the quarantined Spotify companion path). `buildAntiRepetitionDirective(excludedFacts, recentBreakHistory)` stays on the lore clip. Word caps: opening lore ≤32 + announcement ≤13; mid-session lore follows the Host Studio lore tier (`phaseWordCeiling`). Extended deep-dive formats (`directors_cut`, `time_capsule`) use `gpt-4o`; `standard` and `roots_branches` stay on `gpt-4o-mini`. Completions set `frequency_penalty: 0.4` and `presence_penalty: 0.3`. |
| `/api/generate-voice` | POST | TTS dispatch. Live dial defaults to **OpenAI `gpt-4o-mini-tts`** with persona `ttsInstructions` on the `instructions` parameter. Explicit `provider: "local"` POSTs cleaned text to `LOCAL_TTS_URL` (`POST /v1/speech`) and returns **Chatterbox-Turbo speech bytes** (`audio/wav`) with `X-SongHost-Voice-Provider: local`. Missing URL, sidecar down, or GPU/model unavailable **fails closed** (HTTP error, no OpenAI fallback). Health is the sidecar's own `GET /health` (`ok`, `model`, `gpu`, optional `vramNote`) — there is no Next health proxy. Explicit `provider: "elevenlabs"` remains for mothballed WS-7 callers. **There is no `/api/tts` route** — clients use these two. |
| `/api/liner-notes` | POST | Album / track liner notes copy |
| `/api/artist-events` | GET | Ticketmaster local events for DJ mentions |

Script formatting / soft pauses: `src/lib/tts.ts` / `dj-script.ts`. Extended commentary formats instruct the LLM to inject `<break time="300ms"/>` / `<break time="500ms"/>` tags. `prepareTtsSynthesisText()` **strips / softens** those tags to ellipsis cues for OpenAI `gpt-4o-mini-tts`, ElevenLabs, and the local sidecar — raw SSML MUST never reach any provider. OpenAI input is rejected (not truncated) above **2000** characters (`assertOpenAiTtsInputLength` / `OPENAI_TTS_MAX_INPUT_CHARS`).
...
| `lore_facts` | Fact-text rows used **only** to resolve already-served exclusion topics (`id`, optional `artistId` / `albumId` / `trackId`, `factText`, `category`, `createdAt`). Not injected as positive grounding. |
| `user_lore_history` | Per-listener served-fact ledger (`userId` → `users.id`, `factId` → `lore_facts.id`, `servedAt`); indexed on `userId` and `(userId, factId)`. Anti-repetition only. |
...
| TTS voices | All 13 OpenAI `gpt-4o-mini-tts` voices (never gated) plus 4 laptop custom slots in Host Studio (`local:1`–`4`; audition fail-closed). Live radio follows the Host Studio pick (OpenAI or Custom 1–4). Custom on-air needs the local helper; sidecar down skips the break (no OpenAI substitute). | Same 13 OpenAI voices (never gated) plus the same 4 custom slots. **`local`** sidecar is fail-closed on `/api/generate-voice` and `/api/studio/voice-preview` (WS-8 Phase B Chatterbox-Turbo; Phase C picker; Phase D live dial). **Cartesia** is typed (`VoiceProviderId`) but not wired. ElevenLabs is mothballed from the live dial (WS-7). |
| Host persona | Standard Broadcast on-air (`getEffectivePersona` clamp; persisted Pro `activePersonaId` unchanged) | The Guide, The Critic, The Archivist (plus Standard Broadcast). Stable ids remain `warm-companion` / `sarcastic-critic` / `the-musicologist`. |
...
**Anti-Repetition Fact Engine** (`src/lib/dj/factEngine.ts`): anti-repetition **only** — no positive fact-grounding and no DB fact injection into the prompt. `getExcludedFactTopics(userId)` loads previously served `lore_facts.factText` via `user_lore_history`. `getServedFactIds` / `logServedFact` read/write that ledger. `/api/generate-script` prefers caller-supplied `excludedFacts`, else the ledger, and folds last-N aired scripts (`recentBreakHistory`, last 6) into `buildAntiRepetitionDirective(excludedFacts, recentBreakHistory)` in `promptBuilder.ts`.

**Live-LLM lore:** Live YouTube teaching clips use the **legacy** generate-script assembler (no `trackId`; not lore-cache). One contract: persona required move + lore-tier budget + pace copy-cadence. Teaching truth: verified craft **or** general technique without fake proper nouns — never “describe the vibe” as a soft escape. Rotating **commentary styles** (`COMMENTARY_STYLES`, 7 on-air angles) and **musicology pillars** (`MUSICOLOGY_PILLARS`) may colour topic only; they must not override Guide / Critic / Archivist required moves. The rotation is **city-aware** — `weather_vibe` and `local_events` styles are excluded when no broadcast city is known. Companion/Spotify with `trackId` can still hit the separate lore-cache stack — do not assume the two assemblers share one system prompt.
...
| `LOCAL_TTS_URL` | Optional. `"local"` provider on `/api/generate-voice` (default `http://127.0.0.1:7860`). Sidecar-only: `LOCAL_TTS_PORT`, `LOCAL_TTS_DEVICE=cuda`, `LOCAL_TTS_MODEL_DIR`, `LOCAL_TTS_VOICES_DIR`. GPU/model miss **fails closed**. |
...
|------|----------------|------|
| **Voice** | All **13** OpenAI `gpt-4o-mini-tts` voices (`src/types/voice.ts` `VOICE_OPTIONS`: onyx, echo, ash, ballad, cedar, nova, coral, shimmer, sage, marin, fable, alloy, verse) plus **4 laptop custom slots** (`local:1` … `local:4`, Host Studio cards Custom 1–4). Listener pick is `UserPreferences.preferredVoice`. Live dial follows that pick (OpenAI or local sidecar). | Never gated. Free and Pro share the catalog. Custom audition and on-air Custom breaks need the local helper; they never silently play OpenAI. |
| **Persona** | Four **host jobs** in `src/data/personas.ts` (stable ids): **Standard Broadcast** (Free), **The Guide** (`warm-companion`) / **The Critic** (`sarcastic-critic`) / **The Archivist** (`the-musicologist`) (Pro). Lore tier = depth. Pace = how often. Persona = which teacher is in the booth. Each job has a required teaching move, banned empty hype, `systemPrompt` (LLM copy), `goldenExamples` (few-shot), and `ttsInstructions` (TTS delivery). Blind test: from the transcript alone you should usually guess Guide vs Critic vs Archivist. | Pro jobs locked on Free. `getEffectivePersona()` clamps **display and on-air** to Standard Broadcast; persisted `activePersonaId` is **not** rewritten. Old labels (`Warm Companion`, `Sarcastic Critic`, `The Musicologist`) and new slugs (`the-guide`, `the-critic`, `the-archivist`) migrate via `resolvePersonaId()`. Host Studio shows a lock note when a saved Pro persona is present on Free (`HostSettingsModal`). |
| **Vernacular** | Genre-coloured diction layered on the persona. | **WS-3 — not built.** |

**TTS:** Live path derives `ttsProvider` from Host Studio `preferredVoice` in `src/app/page.tsx` (`resolvePreferredVoiceTarget`: OpenAI id → `"openai"`; `local:N` → `"local"` + slot). Model is `OPENAI_TTS_MODEL = "gpt-4o-mini-tts"` (`src/lib/tts.ts`) with a **2000**-character input reject (`assertOpenAiTtsInputLength`). `provider: "local"` on `/api/generate-voice` proxies to `LOCAL_TTS_URL` (Phase B Chatterbox-Turbo sidecar under `tools/local-tts-sidecar/`) and **fails closed** if the sidecar is down — the break is skipped; music continues at 100%; no OpenAI substitute. **Never late talk-over:** each break attempt has a generation token; skip / new station / Host Studio voice-or-script settings abort the fetch and the sidecar job (`POST /v1/cancel` plus connection close). If the voice package is not ready by **20s** after the transition (`VOICE_PACKAGE_DEADLINE_MS`), skip the live break; Custom hosts may air a prerecorded fallback in the gap, then the song starts at 100%. `VoiceNode.play()` refuses a stale or post-release clip. **Prerecorded banks (Pass 2):** 16 welcomes + 16 fallbacks per Custom slot (Harris/Piper/Quinn/Bea), committed under `public/audio/prerecorded/slot-{1-4}/` and served at `/audio/prerecorded/slot-{N}/{id}.wav`. Welcome fires on new playlist/station/listen start. Fallback fires when a live break is not ready / times out / aborts **and** the gap is still open. Never over music. If the fallback file itself 404s, fail closed (silence; no OpenAI substitute). OpenAI follow-up (thin disk set) is not in this pass. Local speech HTTP is also aborted at **28s** (`LOCAL_TTS_SPEECH_TIMEOUT_MS`). Short station-ID lines are cached. Sidecar rejects text over 520 characters, caches speaker conditionals per slot, logs `duration_ms` / abort reason, and **cannot preempt CUDA** — cancel discards the WAV and unblocks HTTP; the next job waits for the current forward to finish. **Phase C:** Host Studio auditions custom slots via `/api/studio/voice-preview?personaId=local:1` (etc.) → sidecar; sidecar down returns a clear error and never synthesizes OpenAI for that request. ElevenLabs stays in-tree (`provider: "elevenlabs"` on `/api/generate-voice`) for admin-only WS-7 Director's Cut pre-renders; it is not on the Host Studio live picker.

**Migration:** `LEGACY_PERSONA_ALIASES` maps old named-host ids (Henry, Sloane, Miles, Devon, Kira, Jasper, Sam/Maya/Alex, …) and job-name slugs / old labels → the 4 live persona ids. `LEGACY_PERSONA_VOICE` is the documented OpenAI fallback **only when a caller has a legacy id and no separate voice**. `preferredVoice` is never rewritten.

**Persona quality gate:** After LLM script generation, `runPersonaScriptQualityGate` (`src/lib/dj/personaJobs.ts`) checks teaching clips for the required job move (Guide ear cue / Critic judgment / Archivist lineage), banned empty hype, and a clone-opener heuristic. One retry with a repair directive; if the retry still fails, air the best attempt. Never hang the gap. Skip session openers, `song_id`, stingers, recaps, and names-only announcement clips. Local TTS stays fail-closed — a failed or late voice package does not talk over music.

**Host Studio display (WS-6 Part A / WS-8 Phase C):** Free player-bar pill shows the selected voice label (`resolveHostDisplayName` + `preferredVoice`) — OpenAI names or Custom 1–4. Pro shows the persona job name (**The Guide** / **The Critic** / **The Archivist**). Cards show a one-line job subtitle from `DjPersona.description`, not adjective soup. Persona picker selection is clamped with `getEffectivePersona`; Pro cards stay locked. Custom 1–4 cards audition the laptop sidecar; OpenAI cards still audition OpenAI.
...
| Level | Behavior |
|-------|----------|
| `talkative` (Every Song) | Host on every track (`full_break`). Scheduler kind is `song_intro` on `standard` commentary format, or Pavlovian `artist_trivia` when an extended format is selected (`directors_cut` / `time_capsule` / `roots_branches`) — concert/weather `local_events` can still take priority. Mid-session `song_intro` is a lore kind (`isLoreSegmentKind`), so playback runs the Pavlovian two-clip path (earcon → live-LLM lore → optional sweeper → announcement in the gap). A station-ID sweeper (`includeStinger`) also plays every 3–5 voiced breaks **alongside** that break. Opener (track 1) is unchanged: single-clip `song_intro`, no earcon, no lore, no stinger. **Copy cadence:** lore/teaching clips teach first and must not open with the incoming title or “Up now is …”. The separate announcement clip leads with up next / now playing / here's — do not default to “You just heard / That was” (the host already talked before the last song). |
| `standard` | Voiced break every 2–4 tracks. **Always tell me what's playing** (`UserPreferences.alwaysAnnounceSongs`, default ON) is scoped to this level only: a running announced ledger tracks which songs had title+artist spoken; silent-gap tracks with a known intro length get a names-only `song_id` (playback announces in the pre-song gap, then starts at 100%) that does **not** reset lore cadence; 2+ still-unnamed prior songs trigger a catch-up recap on the next full break. Toggle OFF is silent gaps with only some songs named. **Copy cadence:** “You just heard / That was” is allowed and encouraged when bridging after silent songs. |
| `music_focused` | Every 5–7 tracks. **Copy cadence:** same lookback openers as Natural Pace when bridging after silent songs. |
...
**Announced ledger (Natural Pace only):** When `alwaysAnnounceSongs` is ON and chatter is `standard`, `SchedulerState.announcedTrackIds` records tracks named in a plan (`song_intro` / `song_id` / `recap` / `up_next` / `artist_trivia` announce lists). A names-only `song_id` marks the track announced and increments `sessionTrackCount` but does **not** reset `tracksSinceLastBreak`, so lore still lands every 2–4 tracks. Playback always announces in the pre-song gap, then starts at full volume. The opener (track 1) is unchanged and starts in the ledger as announced. The toggle is a **global** preference (no station-level override yet).
...
|--------|------|-----------------|----------------------------------------------------------|
| `standard` | Free | Quick broadcast breaks and track intros (default) | Live dial: pause, speak in the gap, start at 100%. Quarantined companion: duration-based Mode A (≤ 15s, relative duck `0.18` / Chill `0.12` / Hyped `0.25`) or Mode B (> 15s, bed `0.25`) |
...
| `directors_cut` | Pro | **80–110 words / ~35–45s.** Required: one concrete craft/history beat **and** one why-it-matters beat, then a clean handoff. Local TTS uses the same length (no GPU soft-cap). Standard stays shorter. | Same DirectStream mix-bus DTS; quarantined companion Mode A/B (format-aware Pause–Talk–Resume is Phase 6) |
...
**Segment-kind routing (not a commentary format):** Session-opening `song_intro` is single-clip with **no earcon** (templated station liner, no LLM). **Every mid-session voiced break** (`song_intro`, `up_next`, `artist_trivia`, `local_events` — `isLoreSegmentKind`) is Pavlovian two-clip (earcon → live-LLM lore in silence → optional stinger → announcement in silence). `song_id` is names-only (never lore, never Pavlovian). Stinger and recap stay single-clip. Earcons: session-opening `song_intro` → none; mid-session `song_intro` / `artist_trivia` / `up_next` → lore earcon; `local_events` weather/concert → respective earcons; `roots_teaser` → teaser earcon; `song_id` / `stinger` / `recap` → none.

Station override wins over the global preference via `resolveStationSettings()`. Live `AudioPlayer` never ducks; it speaks in the gap then starts at 100%. `resolveBreakTransitionPolicy` (`STANDARD_BREAK_DUCK_RATIO` vs `EXTENDED_BREAK_AMBIENT_FLOOR` 0.05 / `pause_or_ambient`) is consumed by **quarantined companion** (`webOrchestrator.ts`) and does **not** drive the live FSM. Format-aware Pause–Talk–Resume on DirectStream is Phase 6.

---

## 9. Key Invariants (Do Not Regress)

1. **First song (live YouTube dial):** pause until audio unlock → `seekTo(0)` → `tryEmitOnPlaying()` once per load → AudioPlayer arms `hard_pause` and pauses for the opener liner → song starts at **100%** after speech. Never `setVolume(18)` for a DJ break. Station select MUST NOT pin `duckBus` to `DUCK_RATIO`. Fail-closed: `releaseLaunchDuck` + 3s playhead watchdog restore to `1.0` when no opener speech is on air.
2. `sessionOpeningDjRef` lives in `AudioPlayer.tsx`. Set `true` only on `stationId` / `queueGeneration` change.
3. Opening DJ is `song_intro` unless `chatterPacing === "music_only"`. It is a **single clip** (templated opener liner) — no earcon, no lore, no LLM, no 500 ms gap. Mid-session `song_intro` is Pavlovian; names-only "what's playing" is the separate `song_id` kind.
4. `silent` / `plan: null` → AudioPlayer must not force a DJ intro.
5. Stabilize audio-hook callbacks in refs; no unstable effect deps.
6. **Host vs music:** Live `AudioPlayer` never ducks. Pause for every voiced break, speak in the gap, start the song at `UNDUCKED_GAIN`. `playDjIntro` defaults `duckMusic` to false. `VoiceNode` duck (`DUCK_RATIO` 0.18 / 300 ms / 1500 ms restore) stays for quarantined companion and tests only. Abandoned openers fail-closed via `releaseLaunchDuck` (600 ms) plus a 3-second playhead watchdog. Live AudioPlayer does **not** call `resolveBreakTransitionPolicy`. Quarantined companion **Mode A**: mood-aware relative ducking (`0.18` default, Chill `0.12`, Hyped `0.25`) over **600 ms** linear, log swell **800 ms** default (Chill `1200 ms`, Hyped `400 ms`). Quarantined companion **Mode B**: ramp to **0** over **1500 ms**, hold station bed at **0.25**, decay **400 ms** before hard-launch. Never duck the voice bus. Format-aware Pause–Talk–Resume on DirectStream is Phase 6.
7. Prefetch plans the break **once**; consumer commits `nextState` at take time. Two-ahead warms N+1 and N+2 from **song start** into `prefetchedBreaksMap` (track id + settings fingerprint). Prefetch buffers stay isolated (`muted` / `volume = 0` before `.src`; never session `AudioContext` / `MediaElementAudioSourceNode`). TRACE 4 **single emitter:** `Prefetch buffer ready` **only** in `VoiceNode.preload()`; `DJ Voice on-air` **only** in `VoiceNode.play()` after confirmed HTML5 `playing` (skip on abort or failed start). Do not re-emit from `dj-intro.ts`, `AudioPlayer.tsx`, or `prefetchEngine.ts`.
...
27. **Pavlovian lore break (WS-6):** Lore-type kinds (`isLoreSegmentKind`: mid-session `song_intro`, `up_next`, `artist_trivia`, `local_events`) run earcon → ~500 ms gap → unducked live-LLM lore → optional stinger → Track B duck 18% / 300 ms → announcement → restore 1500 ms. Session-opening `song_intro` is single-clip with no earcon / no LLM (templated liner over the ducked intro, or short silent station-ID on `hard_pause`). `song_id` is names-only, never Pavlovian. Stinger / recap stay single-clip. Missing earcon skips to lore; failed announcement restores Track B. Full FSM: [AUDIO_ORCHESTRATION_SPEC_2.md](./AUDIO_ORCHESTRATION_SPEC_2.md) §1.
...
| 7 — Extended commentary + fact engine + weather | ✅ / 🔜 | Formats live; lore-type breaks use Pavlovian two-clip (WS-6) with live-LLM lore under `STRICT_TRUTH_GUARDRAIL` (DB positive-grounding reverted; anti-repetition ledger only); DirectStream / YouTube `playDjIntro` uses mix-bus Duck–Talk–Swell for announcement; quarantined companion announcement still uses Mode A 600/800 (follow-up); `lore_facts` / `user_lore_history` exclusion ledger, weather/daypart live; live TTS is `gpt-4o-mini-tts`; Deepgram Aura remaining |
