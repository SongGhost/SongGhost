...
1. `stationId` / `queueGeneration` arms `launchHoldActive` as `intro_ramp`, pins `duckBus` to `DUCK_RATIO`, and calls `setLaunchHold(true, "intro_ramp")` before play/load effects. `DirectStreamProvider.setLaunchHold` does **not** pin duck gain.
