...
| Custom Studio | `studio-` | **Finite authored playlist** — admit the full manifest in native order; never a 1-track dynamic catalog seed. Preserve `spotifyId` / `spotifyUri` from the studio manifest. No catalog replenish; do not wrap to index 0 when the tail drains. |

**Studio vs. dynamic catalog classification:** `isStudioStationId()` is `stationId.startsWith("studio-")`. Studio launches share the **finite playlist** contract with saved custom mixes and Heavy Rotation (`isFinitePlaylistStation` / `isFixedPlaylistStation` in `useStationQueue.ts`): `admitFixedPlaylist(initialTracks)` keeps the authored sequence, `maybeReplenish()` is a no-op, and a drained tail returns `exhausted: true` instead of fetching `/api/recommendations` or `/api/station-tracks`. Do **not** classify `studio-*` as a 1-track starter + catalog-grow station (that path is reserved for preset genre/decade seeds). `studioManifestToStation()` (`src/lib/studio/manifest.ts`) copies native Spotify identifiers onto each `StationTrack` so playback can `playTrack(uri)` without a Search round-trip.

`sessionOpeningDjRef` is set **only** on `stationId` or `queueGeneration` change — never on `videoId` / track advance. `AudioPlayer.handleNewTrack` stamps `trackSessionRef` **after** the `introRunningRef` early-return so an overlapping DJ break does not consume the incoming track's intro slot (retry when the live break releases).
...
**Immutable queue patches (`page.tsx`):** Never assign onto a live `StationTrack` (`track.spotifyUri = uri`). `updateTrackInQueue(index, updates)` clones the row, updates `queueStateRef`, and passes a fresh array to `setQueueState` / AudioPlayer `updateTrackAt` (`applyQueue`) so React 19 re-renders.

**Play/pause glyph:** ControlDeck / Drive Mode / mobile sheet derive `isDisplayPlaying` from `companionPlayback.isPlaying && !isUiPaused()` while a companion source is active. `companionPlayback.isPlaying` is live SDK truth (`!paused` and `progressMs > 0`) — never optimistic. Drive Mode play/pause is `togglePlayPause` only.

**Device readiness:** Companion `playTrack` is gated on a valid Spotify `device_id`. Missing-device / `NO_ACTIVE_DEVICE` results cache URIs in `pendingLaunchUrisRef` and replay from SDK `ready` after transfer. `ensureListening()` calls `spotifyPlayer.activateElement()` and retries stalled launches.

**Station Handoff Invariant:** Station switches MUST synchronously stop/pause prior Connect playback (`haltPriorStreamForHandoff`, never `USER_CLICK_PAUSE`) and arm `suppressCompanionReplayRef` via `AudioPlayer.armStationHandoff()` so `handleNewTrack` cannot race `launchStation` with Spotify Search. `sessionOpeningDjRef` stays armed until the post-reset opener `trackKey` is confirmed (`confirmStationHandoffOpener`). Empty URI resolution / open 429 circuit MUST clear playback and log `[LinerLore] Station launch waiting on URI resolution` — never leave the previous station playing.

**Launch Target Lock Invariant:** `beginStationLaunchLock` without URIs (or with an empty list) MUST set `launchTargetUriRef.current = null`. Keeping the previous station's opener URI causes the SDK listener to ignore state events for the newly launched track.

**Queue VUMeter:** Playlist row equalizers animate only when `index === currentIndex && isPlaying && progressMs > 0`.
...
### Spotify REST 429 circuit breaker & search deduplication

Spotify Web API traffic is funneled through `src/lib/spotify/fetchWithRetry.ts` (`spotifyApiFetch`, `fetchSpotifyGetWithRetry`) from both server routes (`/api/recommendations`, `/api/search`, `src/lib/music/spotify.ts`) and the companion client (`src/lib/player/spotifyRemote.ts`).

**Global 429 circuit breaker** (`spotifyRateLimitResetTime`):

- A live HTTP `429` opens a process-wide backoff window. Default **30 s** when `Retry-After` is absent; otherwise honor the header (delta-seconds or HTTP-date).
- While open, every subsequent Spotify REST call **fail-fasts** with a synthetic `429` and **does not** hit the network. 429s are never retried (502/503/504 still use exponential backoff).
- `isSpotifyCircuitOpen()` is the shared predicate. Search (`searchSpotifyTrackUri`) consults it after the LRU cache and returns `null` immediately when the circuit is open.

**In-memory search deduplication** (`src/lib/player/spotifyRemote.ts`):

- LRU map `searchUriCache` keyed by lowercased `artist:title` (cap **256**). In-flight promises share one GET.
- Negative cache (`searchNegativeExpiry`) holds misses / 429s / circuit-open fail-fasts for **60 s** so identical skip lookups cannot storm Search.
- Native queue `spotifyId` / `spotifyUri` MUST be used first; Search is the fallback when those fields are absent.

Canonical runtime rules: [AUDIO_ORCHESTRATION_SPEC_2.md](./AUDIO_ORCHESTRATION_SPEC_2.md) §1.1.
...
13. **Visibility Guard Isolation:** Tab visibility enforcement applies exclusively when `document.hidden === true` to prevent interference with active in-tab DJ transitions.
14. **Skip Navigation Unlocks:** Manual track advances (SKIP/PREVIOUS) must never be intercepted or immediately re-paused by transport monitoring guards.
15. **Universal Navigation Parity:** Preset stations, Spotify rotation, and custom searches MUST execute the exact same `playTrack(uri)` skip pipeline to guarantee state consistency.
16. **Navigation Lock Immunity:** Transport enforcement guards are strictly locked out from re-stamping pause intent during active track loading windows. `playTrack` / `playSpotifyUri` arm `armSkipNavigationUnlock()` and clear `pausedIntent` before SDK play; `enforcePausedTransport` / `pauseActivePlayer` must not set `pausedIntent = true` or call `pause()` while that unlock is active. Incoming pause calls must not clear an active skip / launch lock.
17. **Source-Tagged Transport Auditing:** Every pause intent evaluation logs its initiating subsystem and explicit acceptance/rejection status.
18. **In-Memory Search Deduplication & Negative Caching:** Spotify track URI resolution (`searchSpotifyTrackUri`) MUST check the LRU `artist:title` memory cache first and fail-fast (`null`, no network) when the global 429 circuit breaker (`spotifyRateLimitResetTime` / `isSpotifyCircuitOpen()`) is active. Misses and 429s are negatively cached (60 s).
19. **Playhead-First Skip Guarantee:** Skip execution (`nextTrack()`) MUST advance `currentIndex` and `applyIndex()` synchronously on the UI thread before triggering background replenishment (`maybeReplenish()`, never awaited on the skip path) or async URI resolution (`resolvePlayableSpotifyUri` / Search). `beginManualNavigationOverride()` / `skipCooldownRef` / `isSkipInFlightRef` MUST be armed on the skip click *before* any `await`.
20. **Preservation of Native Track Identifiers:** Station queue handoffs (`onCompanionPlayTrack` / `launchCompanionTrack` seed), Studio manifest transformations (`studioManifestToStation`), Heavy Rotation / Top Tracks, and Artist Radio / Artist Mix (`/api/artist-radio`) MUST attach and preserve native `spotifyId` and `spotifyUri` at creation time so playback can `playTrack(uri)` without a 30-wide Search burst.
21. **Immutable StationTrack Updates:** `page.tsx` MUST NOT mutate live queue rows in place. Persist Spotify URIs (and any other row patches) via `updateTrackInQueue(index, updates)`, which returns a fresh `StationTrack`, writes `queueStateRef`, and applies a new queue array through `setQueueState` / AudioPlayer `updateTrackAt` (`applyQueue`). In-place `track.spotifyUri = uri` will not re-render React 19 UI.
22. **Play/Pause Glyph Invariant:** Play/pause buttons MUST derive display state from `companionActive ? (companionPlayback.isPlaying && !isUiPaused()) : isPlaying`. Local YouTube `onPlayingChange` is ignored while companion mode is active. Volume changes call `ensureListening({ resumePlayback: false })` and MUST NOT flip `setIsPlaying(true)`.
23. **Drive Mode Transport:** Drive Mode overlay play/pause MUST route through `page.tsx` `togglePlayPause()` / `handlePlayPause` (AudioPlayer `onTransportPlayPause`) — never local YouTube `play()` / `pause()`.
24. **Device Readiness Invariant:** Spotify `playTrack` MUST be gated on a valid `device_id` (`getSpotifyActiveDeviceId()`). A missing device or `NO_ACTIVE_DEVICE` caches the target URIs (`pendingLaunchUris` / `pendingLaunchUrisRef`) instead of dropping the command. `onSdkReady` MUST replay those URIs after `setSpotifyActiveDeviceId` + `transferPlaybackToLocalDevice` succeed. `ensureListening()` / `unlockAudioContext()` MUST call `spotifyPlayer.activateElement()` on user gestures and retry stalled launches.
25. **Queue VUMeter Invariant:** Queue row indicators and `<VUMeter />` MUST animate only when `index === currentIndex && isPlaying && progressMs > 0`. `companionPlayback.isPlaying` is derived from live SDK state (`!paused` and `progressMs > 0`) — never stamped `true` before audio is confirmed. A stalled or 0:00 playhead shows a static `▶` / paused glyph.
26. **Station Handoff Invariant:** Station switches MUST synchronously stop/pause prior Connect playback (`haltPriorStreamForHandoff`, never `USER_CLICK_PAUSE`) and arm `suppressCompanionReplayRef` so `AudioPlayer.handleNewTrack` cannot burn Search ahead of `launchStation`. `sessionOpeningDjRef` is preserved until the post-reset opener `trackKey` is confirmed. Empty URI lists / open 429 circuits clear audio and log `[LinerLore] Station launch waiting on URI resolution`.
27. **Launch Target Lock Invariant:** `beginStationLaunchLock` without URIs MUST reset `launchTargetUriRef.current = null` so leftover opener URIs from the previous station cannot ignore SDK state events for the newly launched track.
