...
**Manual Navigation Override:** User-initiated SKIP or PREVIOUS commands MUST clear all `isPaused` locks and suppress transport pause enforcement for 2000ms (`skipCooldownRef`) to allow remote SDK playback stabilization.
...
1. `useWebOrchestrator` listens for `visibilitychange` and window `blur`. The guard MUST ONLY execute when `document.hidden === true` or on an explicit window `blur`. If UI pause intent is set in that background state, call `spotifyPlayer.pause()` and/or `audioContext.suspend()`.
...
**In-Tab Exemption:** The background visibility guard MUST ONLY execute when `document.hidden === true`. It is strictly forbidden from running or enforcing pause locks during active FSM transitions or DJ speech completions while the browser tab is visible.

**Skip Navigation Unlock:** Rule 3 MUST NOT run for 2000ms after `playTrack` / SKIP / PREVIOUS (`armSkipNavigationUnlock` / `skipCooldownRef`). Transport monitors ignore leftover `isPaused` so the Spotify Web SDK can buffer without an immediate `spotifyPlayer.pause()`. `enforcePausedTransport()` MUST NOT re-stamp `pausedIntent` or issue `pause()` in this window. Incoming `pause()` / `enforcePausedTransport()` calls MUST NOT clear an active navigation lock while a skip or station launch is buffering.

#### Transport & Subsystem Isolation

**Pipeline Isolation Invariant:** Voice pipeline (TTS) teardown MUST NOT issue transport commands or state mutations to the Music pipeline (Spotify SDK). Resetting or stopping HTML5 TTS speech may only `pause()` / `currentTime = 0` the speech player — never `WebOrchestrator.pause()`, `pauseActivePlayer()`, or Spotify REST pause endpoints.

**Tagged Telemetry Rule:** All `pause()` calls MUST pass a `source` tag parameter for auditability (e.g. `USER_CLICK_PAUSE`, `USER_DECK`, `DJ_CLEANUP`, `TAB_BLUR`, `SDK_GHOST_PLAY`). Each pause intent evaluation MUST log `[LinerLore PAUSE_ACCEPTED]` or `[LinerLore PAUSE_REJECTED]` with the initiating subsystem.

#### Control Deck Actions

**Skip Isolation Invariant:** Skip handlers MUST NOT invoke `pause()` or set `isPausedIntent = true`. Pause intent from the UI deck is strictly reserved for explicit user pause clicks (`USER_CLICK_PAUSE`). Skips, station launches, and track transitions MUST arm `isTransitioningLock` instead.
...
**Title Display:** Deck text containers must dynamically occupy available horizontal flex space and provide full HTML hover tooltips for truncated strings.
