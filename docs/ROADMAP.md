...
| **WS-8 — Local custom TTS beside OpenAI** | **Phase A + B + C + D done. Speed + safety pass done.** `TtsProvider` / `VoiceProviderId` include `"local"`; `/api/generate-voice` proxies to `LOCAL_TTS_URL` (`tools/local-tts-sidecar/` Chatterbox-Turbo on the laptop GPU) and fails closed (no OpenAI fallback). Host Studio lists 13 OpenAI voices plus Custom 1–4 (`preferredVoice` = OpenAI id or `local:1` … `local:4`); custom audition is fail-closed. **Live dial follows the Host Studio pick** (OpenAI voice id → openai; `local:N` → local + voiceSlot) for the session opener and between-song breaks. Prefetch stamps the same provider/voice. Custom selected + sidecar down skips the break — no silent OpenAI substitute. Gap-then-100% unchanged. Local Director's Cut warms at **120s**, uses a **48–62 word** GPU budget (OpenAI stays 80–110), aborts sidecar HTTP at 28s, and never plays a late clip over YouTube. |
...
- [x] Duck vs Pause Transition Rules — live YouTube / `AudioPlayer` pauses for every voiced break, speaks in the gap, then starts the song at 100% (`duckMusic: false`). Mix-bus `DUCK_RATIO` ducking remains in `VoiceNode` for quarantined companion and tests only. Format-aware pause-or-5% ambient (`resolveBreakTransitionPolicy` / `EXTENDED_BREAK_AMBIENT_FLOOR`) is consumed by quarantined companion only; Pause–Talk–Resume on DirectStream is Phase 6
...
- [x] Anti-Repetition Fact Engine (`lore_facts` / `user_lore_history` served-fact ledger & negative prompt injection only — no positive DB fact-grounding)
- [x] Live-LLM lore — lore clips are generated live by the LLM under `STRICT_TRUTH_GUARDRAIL` (no invented biography; vibe/production/chart context only); all 7 Commentary Styles rotate (city-aware: weather/local-events excluded without a broadcast city); Musicology Pillars rotate; every mid-session voiced break is Pavlovian; `song_id` names-only kind for "always tell me" (long-intro duck / short-intro pre-song gap announce); Silent grays Host Studio
...
#### DJ Lore — how it evolved

DJ intros started as templated liners (a short written line, no AI). A later lore revival made mid-session host breaks feel like a real radio bit: a short chime, then a lore clip in the silence between songs, then an optional station sweeper, then the host naming the incoming track. Live YouTube now speaks in the gap, then starts the song at 100% — it does not talk over a ducked bed. That round also added a simple “always tell me what’s playing” song ID, rotating Commentary Styles (skipping weather and local styles when no city is set), and wired the mid-session chime. A database of verified facts was tried as a grounding layer and then taken out — lore is now written live by the AI under a strict truth rule (no invented biography; vibe, production, or chart context only). **Director's Cut depth fix:** lore tiers are now a real insight-volume knob. Mid-session Director's Cut teaches 2–3 beats (~80–110 words / ~35–45s) instead of being flattened to one trivia line by the old Standard brevity caps; Time Capsule and Roots sit between Standard and Director's Cut. Long Breaks remains frequency only. **Local Custom voices** use a shorter Director's Cut (48–62 words) so the laptop GPU can finish in the gap; OpenAI Director's Cut is unchanged.
...
## Direction — not scheduled for the current pre-production round

These are product directions only. They are **not** committed milestones, have **no** dates, and must not be treated as the current workstream.

### Pocket / Drive mode

**Direction — near-term; design properly before building.** Not a committed milestone; no dates.
