...
DJ intros started as templated liners (a short written line, no AI). A later lore revival made mid-session host breaks feel like a real radio bit: a short chime, then a lore clip in the silence between songs, then an optional station sweeper, then the host naming the incoming track over ducked music. That round also added a simple “always tell me what’s playing” song ID, rotating Commentary Styles (skipping weather and local styles when no city is set), and wired the mid-session chime. A database of verified facts was tried as a grounding layer and then taken out — lore is now written live by the AI under a strict truth rule (no invented biography; vibe, production, or chart context only).
