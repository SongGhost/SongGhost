...
**Status: RESOLVED Aug 26 2026, then REGRESSED; superseded by T36 (Aug 27 2026).** Original fix: `DriveModeOverlay` (`fixed inset-0 z-[200]`) hid the dock (`z-50`) and its 320×200 YouTube viewer. Dock z became conditional (`z-50` normally, `z-[210]` while Drive Mode on). Larry's Aug 27 test showed the video was still covered, so T36 replaces this with a fixed top-center iframe window + overlay restructure. See T36 for the current approach.
...
**T36 — Drive Mode live YouTube window + mobile X fix (supersedes T22).**
**Status: RESOLVED (Aug 27 2026).** T22's conditional-z dock fix regressed (Larry's Aug 27 test: video still covered in Drive Mode). New approach: while Drive Mode is on, the existing YouTube iframe host (`containerRef` in `AudioPlayer.tsx`) is promoted via className only to `position: fixed; top: safe-area; centered; z-[210]` (above the overlay's z-200), sized 196×110 mobile / 248×140 desktop, capped at `min(196px, 100vw-160px)` so it can never overlap the X on narrow phones. The iframe is NOT re-parented or remounted (no reload, no audio interruption). `DriveModeOverlay` drops the big album-art square; layout is now a top spacer (video-sized) → title/artist (small Host Live pill when DJ speaking) → transport controls. Mobile X root cause: the old art square overflowed upward over the header and covered the X (not a touch handler / stale closure). Fixed by removing the art square + giving the header and X `z-10` and `touch-action: manipulation`. Wake-lock and `setDriveMode` behavior unchanged. This is a layout fix, not a claim of full YouTube TOS compliance. Code: `src/components/AudioPlayer.tsx`, `src/components/studio/DriveModeOverlay.tsx`.
